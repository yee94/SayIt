...
SayIt can use `sherpa-onnx` for local offline ASR through its C API. The default repository build does not bundle the runtime; the code path is compiled only when `SHERPA_ONNX_AVAILABLE` is enabled locally.
...
3. Build SayIt with the shared scheme.
...
- Model downloads are managed by SayIt and stored under the same configurable model storage root as other local models.

## Integration Notes

- `SayIt/SayIt-SherpaOnnx-Bridging-Header.h` imports `sherpa-onnx/c-api/c-api.h`.
...
- The SayIt app target still has a no-op-by-default build phase that can copy `libonnxruntime*.dylib` into the app bundle if a future dynamic runtime package is used and `SHERPA_ONNX_COPY_RUNTIME=YES`.

## Release Rule

Do not commit generated `sherpa-onnx` binaries into this repository. If SayIt ships sherpa-onnx in a release, pin an upstream sherpa-onnx commit or release, archive the exact runtime artifact in the release process, and run real FireRed and FunASR Nano recognition smoke tests before publishing.
