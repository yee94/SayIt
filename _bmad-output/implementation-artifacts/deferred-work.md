...
## 来自 spec-bluetooth-audio-connecting-hud.md（2026-07-29）

### 1. 原生录音时长包含设备初始化耗时

- **发现**：Rust 录音计时从设备选择与 stream 建立前开始，而 HUD 计时在 `start_recording` ready 后才开始。蓝牙设备连接较慢时，储存的录音时长会大于使用者看到的拾音时长，也可能影响过短录音判定与用量统计。
- **延后理由**：这是既有后端计时边界，修正需要改动 Rust 录音生命周期；本次明确不调整 Rust ready 协议，并且连接 HUD 的视觉验收不依赖储存时长。
- **后续切入点**：把 Rust 的录音起始时间移动到 stream `play()` 成功之后，并补设备初始化延迟下的时长合约测试。
