...
- 词典改用独立 CSV 文件持久化：首次启动会自动迁移既有 SQLite 词条，手动添加、AI 学习、编辑和删除会立即安全写入，词典数据也可直接备份和查看。
- iCloud / 共享文件夹词典同步：设置中选择同步目录后，各设备会在启动、词典变化和定时检查时合并快照；相同词条保留最高权重，手动词条优先保留。

### Improved

- 刘海 HUD 的长文本截断保持最新内容可见，省略号与文字对齐更稳定。
...
- 刘海 HUD 在贴上完成和取消操作后会立即从顶部缩回，不再展示成功对勾或「已取消」文字；结束动画更紧凑，收起过程保持明确的顶部锚点。

## [0.12.10] - 2026-07-30

### Added

- LLM 自定义请求 Header：可在设置中填写 JSON Header，用于接入自建网关、代理和兼容服务；应用自动保留鉴权与内容类型等核心请求头
- 实时识别字幕稳定分段：已确认的语句保持稳定，正在修正的尾段以辅助下划线显示，语音输入过程更容易跟读

### Improved

- 刘海 HUD 动效与波形节奏：录音、转写和收缩过程围绕顶部锚点平滑展开，波形事件频率同步优化，降低跨进程事件压力

## [0.12.9] - 2026-07-29

### Added

- macOS 应用内自动更新验证版本：供已安装 0.12.8 的用户测试无须重新下载安装的升级流程
...
- 麦克风连接状态提示：按下快捷键后，HUD 会先显示「正在连接麦克风」，清楚反馈录音设备的启动进度

### Fixed

- Windows 与 macOS 发布构建：统一目标窗口捕获命令的跨平台返回路径，满足两端 Rust Clippy 检查，三平台构建可继续执行

## [0.12.6] - 2026-07-29

### Fixed

- 智能学习通知与录音字幕叠层：进入录音、转写或整理时，「已学习」提示会重新排队，等语音流程结束后再展示，不再盖住实时字幕
- 词典学习在 HUD 隐藏时被延迟到下次录音：学习落库与通知改为下一事件循环调度（不再依赖 `requestAnimationFrame`），窗口隐藏时也能及时完成

### Improved

- 词典页合并为统一词表：AI 推荐词以机器人图标标识，不再分「AI 推荐 / 手动添加」两区
- 产品命名统一为「词典」：侧栏、功能介绍与文档中的「自定义字典」均已替换
...
- 词典管理效率提升：词条支持原地编辑、空值和重复词校验，学习成功的 HUD 提示同步显示已加入的词
...
- Typeless 词典一键导入（macOS）：可读取本机已登录的 Typeless 词典，批量加入 SayIt 词典；既有词条与大小写重复项会自动跳过，Windows 会显示平台支持提示
